      ...
      <div class="wizard">
         ...
         <div>
            <div class="tab-content">
                ...
                <div class="external-event bg-{{$a}}" data-duration="{{$row->time_duration?$row->time_duration:''}}"
                 data-type="{{$row->activity_type?$row->activity_type:''}}" 
                 data-detail='{{$row->activities_details?$row->activities_details:""}}'>{{$row->activity_name}}</div>
                                       @endif
                                    @endforeach
                                   @endif
                                 </div>
  ...
  $(document).on('click','.fCalendar-wrapper',function()
  {
              ...
              var newPopover=$($("#popoverContent").html());
               ...
               // })
                ...
                event.evtData.forEach(function(elem,index){
                  if(elem.id == "island_hoping")
                    newPopover.find('.popover-oldFields [name="island_hoping"]').val(elem.value);
                  else if(elem.id == "include")
                    newPopover.find('.popover-oldFields [name="include"]').val(elem.value);
                  else if(elem.id == "doesntinclude")
                    newPopover.find('.popover-oldFields [name="doesntinclude"]').val(elem.value);
                  else
                    newPopover.find('.popover-newFields').append(setNewField(elem))
                  console.log(elem)
                  // console.log(index)
                })
              return newPopover;
    ...
    function saveEvent(popoverclick){
      ...
      $popupWrper.find('.popover-oldFields p,.popover-newFields p').each(function(){
        allPopoverData.push({label: $(this).find('.lbl-name').text(),value:$(this).find('textarea').val(),id:$(this).find('textarea').attr('name')});
      })
...
function setNewField(newElemIndex){
...
var newElemIndex=Math.floor((Math.random() * new Date().getTime())).toString();
  $newElement=$('<p class="mt-10" id="'+id+'"><span class="removeNewField">&times; </span><span class="lbl-name" contenteditable>'+label+' </span> : <textarea class="form-control text__editor" name="'+id+'" rows="2" cols="80">'+value+'</textarea> </p>');
...
var newElemIndex=Math.floor((Math.random() * new Date().getTime())).toString();
  $newElement=$('<p class="mt-10" id="'+newElemIndex+'"><span class="removeNewField">&times; </span><span class="lbl-name" contenteditable>Add label name </span> : <textarea class="form-control text__editor" name="'+newElemIndex+'" rows="2" cols="80"></textarea> </p>');
  $newElement.find('[contenteditable]').blur(function(){
      var text='';
      text=$(this).text();
      if(!text){
