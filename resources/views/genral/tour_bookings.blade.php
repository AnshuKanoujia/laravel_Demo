                     ...
                     @foreach($get_all_tours as $value)
                      <tr id="row_{{$value->id}}">
                       ...
                       <td class="status">
                        ...
                        @elseif($value->booking_status=='3') <span class="btn btn-danger btn-xs btn-block">Cancelled</span>
                        ...
                        <select  onchange="update_status(this,{{$value->id}},{{$value->external_tour_cost}})"> 
                          ...
                          <option @if($value->booking_status=='3') selected @endif value="3">Cancelled</option>
          ...
          if(confirm('Do you want  to  delete this?'))
          {
              $.ajax({
                  ...
                  success: function(result){
                      ...
                      if(result=='200'){
                        $('#row_'+rowId).remove(); 
                        $('.alertmessage').html('<span class="alert alert-success">Booking deleted...</span>');  
                      }
                      else{ $('.alertmessage').html('<span class="alert alert-danger">Somthing event wrong!...</span>'); }
                  }
              });
          }
      }
