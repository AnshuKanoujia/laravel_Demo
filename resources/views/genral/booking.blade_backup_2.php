				...
				// console.log(passenger, tour_id, 'ajaxHit------------------')
				  $.ajax({
						...
						success: function(result){
							...
							// $('#financial-details').on('shown.bs.modal', function () {

														//    var $table = $('#financial-details').find('table').eq(0),
														// 		$thead = $('<thead>'),
														// 		$tbody = $('<tbody>'),
														// 		$tr = $('<tr>'),
														// 		$td =$('<td>'),
														...
														// 		$select.append(
														// 			$('<option>',{
														// 				'data-airportCost': data.airport_cost_internal,
														// 				'data-id': 	data.id,
														// 				'data-taxiCost': 	data.taxi_cost_internal,
														// 				'data-tourCost': 	data.tour_cost_internal,
														// 				'value': data.seats
																		
														// 			}).text(data.title)
                                                   
														// 		).appendTo($td.appendTo($tr.appendTo($tbody.appendTo($table))));
														// 	})
...
function createTableDom(key, mainData, all_taxi){
	...
	//Car
	$td =$('<td>'),
	$select = $('<select>',{'class': 'form-control','multiple':'multiple'});
