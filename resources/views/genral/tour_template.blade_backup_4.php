   ...
   <div class="modal-dialog modal-sm">
	...
	<div class="modal fade" id="financial-details" role="dialog">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title">Add Financial Details</h4>
				</div>
				<div class="modal-body">
					...
					<table class="table table-striped">
						...
						<thead>
							<tr> 
								<th>No of Pax</th> <th>Supplier Cost</th> <th>Supplier</th> 
								<th>Extras Per Pax</th><th>Extras Per Car</th>
								<th>Action</th>
							</tr>
						</thead> 
						...
						<tbody> 
							<tr> 
								<td scope="row">
									<input type="number" name="no_of_pax" class="form-control" placeholder="No of pax" />
								</td>  
								<td>
									<input type="number" name="supplier_cost" class="form-control" placeholder="Supplier Cost" />
								</td> 
								<td>
									<div class="d-flex">
										<input type="text" name="supplier_name" class="form-control" placeholder="Name" />
										<input type="text" name="supplier_value" class="form-control" placeholder="Value" />
										<button class="btn btn-primary add-supplier">
								...
								<td>
									<input type="number" name="extras_per_pax" class="form-control" placeholder="Extras Per Pax" />
								</td>
								<td>
									<input type="number" name="extras_per_car" class="form-control" placeholder="Extras Per Car" />
								</td>
								<td>
									<button class="btn btn-primary add-new-row">Add New</button>
								</td> 
							</tr>  
						</tbody>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary cancelDropEvt" data-dismiss="modal">Cancel</button>
					<button type="button" class="btn btn-success submit-finance">
						<span class="fa fa-spin"></span>
					Submit</button>
			</div>
		</div>
...
<script src="https://cdn.ckeditor.com/4.4.3/standard/ckeditor.js"></script>
 <script>
	$(document).ready(function(){
		...
		$('.add-supplier').on('click', function(e){
			e.preventDefault();
			$(this).closest('td').append(
				$(this).closest('.d-flex').clone(true)
			);
           $(this).closest('.d-flex').find('button').remove();
		});
		...
		$('.submit-finance').on('click', function(){
			var arr = [], obj={};
			var supplyArr = [];
			$('#financial-details tbody tr').each(function(i){
				$(this).find('td').each(function(i){
					
					if($(this).find('input').length){
						if(i == 2){
							obj['Supplier'] = supplyArr;
							$(this).find('.d-flex').each(function(){
                var supplyObj = {};
								$(this).find('input').each(function(){
                                console.log($(this).attr('name'),$(this).val())
									supplyObj[$(this).attr('name')] = $(this).val();
                 
								});
							  supplyArr.push(supplyObj);  
							})	
