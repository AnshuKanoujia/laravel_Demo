                ...
                <div class="box-header">
                  <div class="box-body">
                        <div class="col-md-12">
                            ...
                            <form action="{{url('upload_images')}}" class="dropzone" method="post" >
                            {{ csrf_field() }}
                            <input  type="hidden"  name="gellery_id" value="{{$driver_id}}" />
                            </form>
                            </div>
                            <div class="form-group">
                                <a href="{{url('upload_confirm')}}" class="btn btn-primary btn-sm pull-left">Skip</a>
                                <a href="{{url('upload_confirm')}}" class="btn btn-primary btn-sm pull-right" >Upload Document</a>
                            </div>
                    ...
                    <tbody>
                     @foreach($get_driver_documents as $value)
                      <tr id="row_{{$value->id}}">
                        ...
                        <a target="_blank" href="{{ URL::asset('images/'.$value->documents)}}"><img src="{{ URL::asset('admin/images/pdf.png')}}" style="height:50px; width:50px;" ></a>
                        @else
                        <a target="_blank" href="{{ URL::asset('images/'.$value->documents)}}"><img  src="{{ URL::asset('images/'.$value->documents)}}" style="height:50px; width:50px;"></a>
                        @endif
                        </td>
                        ...
                        <td>
                        <a href="javascript:void(0);" title="delete" onclick="delete_documents({{$value->id}},{{$driver_id}});" ><i class="fa fa-trash text-danger"></i></td>
                      </tr>
                      @endforeach
...
@section('customjs')
          ...
          $('.edit-on-click').click(function() {
              ...
              .focusout(function() {
                ...
                if(row_id)
                {
                  ...
                  $.ajax({
                          type: "POST",
                          url: "{{url('update_documents_title')}}",
   ...
   $(function() {
       ...
       success: function(file, response){
              ...
              {
                var url="<?php echo URL::asset('admin/images/'); ?>"; 
                var url2="<?php echo URL::asset('images/'); ?>"; 
               /*  $('tbody').append('<tr><td>G</td><td>H</td><td>I</td></tr>');*/
                var html='<tr id="row_'+response.row_id+'" class="even" >';
                      ...
                      {
                        html+='<td class="  sorting_1"><a target="_blank" href="'+url2+'/'+response.filename+'"><img src="'+url+'/pdf.png" style="height:50px; width:50px;" ></a></td>'; 
                      }
                      else
                      {
                        html+='<td class="  sorting_1"><a target="_blank" href="'+url2+'/'+response.filename+'"><img src="'+url2+'/'+response.filename+'" style="height:50px; width:50px;" ></a></td>'; 
                      }
                      ...
                      html+='<td class="">\
                        <a href="javascript:void(0);" title="delete" onclick="delete_documents('+response.row_id+','+response.driver_id+');" ><i class="fa fa-trash text-danger"></i></td>';
                        //$value->id   $driver_id 
                     html+='</tr>'; 
                $('tbody').append(html); 
              } 
   ...
   function delete_documents(rowId,driver_id)
      {
          if(confirm('Do you want  to  delete this?'))
          {
              $.ajax({
                  ...
                  data: {'row_id':rowId,'driver_id':driver_id,"_token":"{{ csrf_token() }}"},
                  success: function(result){
                      //console.log(result);
                      if(result=='200'){
                        $('#row_'+rowId).remove();
