...
<body >
                   ...
                   @if(!empty($get_tour_data->no_of_day))
                       ...
                       @for ($i = $m; $i <= $count; $i++)
                         ...
                         @if($i!=$count)
                            <div class="col-md-4">
                                <div class="box-data">
                                    <h6 contenteditable="" spellcheck="false" >Day {{$i}} <!--{{$get_all_days[$i-1]['start_date']}}--> </h6>
                                    <h4 contenteditable="" spellcheck="false" > Island Hopping <!--{{$get_tour_events_data[$i]->booking_id}}--> </h4>
                                    @if(!empty($get_tour_events_data) )
                                      <ul>
                                      <h5 contenteditable="" spellcheck="false" >Includes:</h5>
                                      <?php  $start=''; $selfdrive='';  $lunch="";  $place='';  $end=''; $allplace='';  ?>
                                        @foreach($get_tour_events_data as  $value)
                                          ...
                                          @if($get_all_days[$i-1]['start_date']==date('Y-m-d',strtotime($value->start_date)))
                                            <!-- {{strtotime(substr($eventdata->start,0,10).' '.substr($eventdata->start,11,8))}} -->
                                           <?php if($start==''){
                                               $start=substr($eventdata->start,0,10).' '.substr($eventdata->start,11,8); 
                                           }else{
                                              if(strtotime($start) > strtotime(substr($eventdata->start,0,10).' '.substr($eventdata->start,11,8)))
                                              {
                                                  $start=substr($eventdata->start,0,10).' '.substr($eventdata->start,11,8); 
                                              }
                                           }

                                           if( array_key_exists("lunchYN",$eventdata) && $eventdata->lunchYN=='Yes')
                                           {
                                             $lunch=' Lunch  at '.$eventdata->title; 
                                           }

                                           if($eventdata->title!='')
                                           {
                                               $place=$eventdata->title; 
                                               $allplace.=$eventdata->title.', ';
                                           }
                                            if( array_key_exists("selfDrvYN",$eventdata) && $eventdata->selfDrvYN=='Yes')
                                            {
                                              $selfdrive=' self drive '; 
                                            }

                                           if($end==''){
                                            $end=substr($eventdata->end,0,10).' '.substr($eventdata->end,11,8); 
                                            }else{
                                           if(strtotime($end) < strtotime(substr($eventdata->end,0,10).' '.substr($eventdata->end,11,8)))
                                           {
                                               $end=substr($eventdata->end,0,10).' '.substr($eventdata->end,11,8); 
                                           }
                                        }
                                        ...
                                        @endforeach
                                         ...
                                         <li>
                                            <img src="{{ URL::asset('public/admin/tourpdf/images/arrow.png')}}"><span contenteditable="" spellcheck="false" >Drive start  at {{date('h:i A',strtotime($start))}}</span>
                                           </li>

                                           @if($selfdrive!='')
                                           <li>
                                            <img src="{{ URL::asset('public/admin/tourpdf/images/arrow.png')}}"><span contenteditable="" spellcheck="false" >Car rental automatic (self drive) for duration of your stay</span>
                                           </li>
                                           @endif

                                           @if($lunch)
                                           <li>
                                            <img src="{{ URL::asset('public/admin/tourpdf/images/arrow.png')}}"><span contenteditable="" spellcheck="false" >{{$lunch}}</span>
                                           </li>
                                           @endif
                                           ...
                                           <li>
                                            <img src="{{ URL::asset('public/admin/tourpdf/images/arrow.png')}}"><span contenteditable="" spellcheck="false" >Enjoy the  at {{$place}}</span>
                                           </li>
                                           <li>
                                            <img src="{{ URL::asset('public/admin/tourpdf/images/arrow.png')}}"><span contenteditable="" spellcheck="false" >Finish at {{date('h:i A',strtotime($end))}} and drive back to Hotel</span>
                                           </li>

                                        <h5 contenteditable="" spellcheck="false" >You will visit :</h5>
                                        <li> <span class="block" contenteditable="" spellcheck="false" >{{$allplace}}</span></li>
                                        </li>
                                      </ul>
                                    @endif
    ...
    <a href="#" class="float" id="btnExport" > <!--id="btnExport" -->
    ...
    <script type="text/javascript" src="{{ URL::asset('public/admin/tourpdf/js/script.js')}}"></script>
</body>
</html>