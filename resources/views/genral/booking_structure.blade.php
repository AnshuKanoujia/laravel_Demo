  ...
  $.ajax({
      ...
      success: function(resp){
        ...
        setTimeout(function(){
          ...
          gettingSettingEvents(moment().format(),false);
...
.on('click','.fc-button-group .fc-prev-button',function(){
  ...
  var defDate=moment(fcActiveDate).subtract(0,'days').format();
    ...
    gettingSettingEvents(defDate,setOld);
})
.on('click','.fc-toolbar .fc-today-button',function(){
  console.log('licked')
    $('.fc-overlay').addClass('active');
    // var defDate=moment().format();
    // gettingSettingEvents(defDate,false);    
