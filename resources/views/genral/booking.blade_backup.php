                              ...
                              <div class="box-body">
                                 ...
                                 @if(count($booking_deatils['get_all_activities']))
                                   @foreach($booking_deatils['get_all_activities'] as $row)
                                       <!--{{$row->activities_details?$row->activities_details:''}}-->
                                       @if($row->activity_type==$value->id) 
                                       <div class="external-event bg-{{$a}}" data-duration="{{$row->time_duration?$row->time_duration:''}}" data-type="{{$row->activity_type?$row->activity_type:''}}" data-detail='[{"label":"Island Hopping:","value":"werwe","name":"island_hoping"},{"label":"Includes :","value":"werew","name":"include"},{"label":"Does not include :","value":"erewr","name":"doesntinclude"},{"label":"rwerewr","value":"werwe","name":"newElem2020"}]'>{{$row->activity_name}}</div>
                                       @endif
...
<script>
var currentEvent,saveEvtObj={},$currentPopupDetailWrapper='',$currentPopoverToOpt='';
  $(function () {
